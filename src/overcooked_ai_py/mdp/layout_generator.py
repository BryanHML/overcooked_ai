...
class Grid(object):
    ...
    def terrain_at_loc(self, location):
        # #OWN Use wrapped location for infinite behavior
        # wrapped_loc = self.wrap_location(location)
        # x, y = wrapped_loc
        # return self.mtx[x][y]
        x, y = location
        return self.mtx[x][y]
    ...
    def is_in_bounds(self, location):
        # #OWN Infinite grid: All locations are considered "in bounds"
        # return True
        x, y = location
        return x >= 0 and y >= 0 and x < self.shape[0] and y < self.shape[1]

    # #OWN DEFINITION REMOVE IF NEEDED 
    # def wrap_location(self, location):
    #     """Wraps a location to simulate infinite grid behavior."""
    #     x, y = location
    #     x = x % self.shape[0]  # Wrap horizontally
    #     y = y % self.shape[1]  # Wrap vertically
    #     return x, y
