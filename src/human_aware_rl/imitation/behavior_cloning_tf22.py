...
def _get_observation_shape(bc_params):
    """
    Determine the observation shape, ensuring consistency across all layouts in bc_params["data_params"]["layouts"].
    """
    base_ae = _get_base_ae(bc_params)
    base_env = base_ae.env
    dummy_states = [
        base_env.mdp.get_standard_start_state()
        for layout in bc_params["data_params"]["layouts"]
    ]
    obs_shapes = [
        base_env.featurize_state_mdp(state)[0].shape for state in dummy_states
    ]

    # Ensure all observation shapes are identical
    assert all(shape == obs_shapes[0] for shape in obs_shapes), "Observation shapes differ across layouts!"
    return obs_shapes[0]
...
def load_data(bc_params, verbose=False):
    """
    Load data from all layouts specified in bc_params["data_params"]["layouts"].
    """
    layouts = bc_params["data_params"]["layouts"]
    combined_inputs, combined_targets = [], []

    for layout in layouts:
        layout_params = bc_params["data_params"].copy()
        layout_params["layouts"] = [layout]
    ...
    if bc_params["use_lstm"]:
        seq_lens = np.array([len(seq) for seq in combined_inputs])
        seq_padded = _pad(
            combined_inputs,
            default=np.zeros((len(combined_inputs[0][0],))),
        )
        targets_padded = _pad(combined_targets, default=np.zeros(1))
        ...
        return np.vstack(combined_inputs), None, np.vstack(combined_targets)
